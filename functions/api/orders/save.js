...
export async function onRequest(context) {
  ...
  try {
    ...
    // UAT sandbox auto-confirms without a real Razorpay payment, so there is
    // no signature to verify — skip the check. Gated on UAT_MODE, so prod
    // (UAT_MODE unset) always verifies the live signature.
    if (payment_method !== 'cod' && env.UAT_MODE !== 'true' && env.RAZORPAY_KEY_SECRET) {
